...
import { LayoutDashboard, MessageSquare, Users, CalendarDays, LogOut, Menu, X, MessageCircle, FileEdit, Ticket } from "lucide-react";

const sidebarLinks = [
  { label: "Dashboard", href: "/admin-dashboard", icon: LayoutDashboard },
  { label: "Website Editor", href: "/admin-dashboard/editor", icon: FileEdit },
  { label: "Messages", href: "/admin-dashboard/messages", icon: MessageSquare },
  { label: "Live Chat", href: "/admin-dashboard/chat", icon: MessageCircle },
  { label: "Bookings", href: "/admin-dashboard/bookings", icon: CalendarDays },
  { label: "Users", href: "/admin-dashboard/users", icon: Users },
  { label: "Promo Codes", href: "/admin-dashboard/promo-codes", icon: Ticket },
];
    ...
    <div className="min-h-screen bg-muted/30 flex">
      ...
      >
        ...
        <div className="p-3 border-t border-border">
          <button
            onClick={logout}
            className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-muted-foreground hover:bg-destructive/10 hover:text-destructive transition-colors w-full touch-manipulation"
          >
            <LogOut size={18} />
            Logout
          </button>
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        <header className="h-14 bg-card border-b border-border flex items-center px-4 lg:px-6 gap-3">
          <button
            className="lg:hidden text-foreground p-1 touch-manipulation"
          ...
          <span className="ml-auto text-xs font-semibold px-2.5 py-1 rounded-full bg-primary/10 text-primary border border-primary/20">
            Admin
          </span>
        </header>
